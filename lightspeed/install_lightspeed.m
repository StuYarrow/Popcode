...
elseif ismac
	...
	options.COMPFLAGS = '-fno-common -no-cpp-precomp -arch x86_64 -isysroot /Developer/SDKs/MacOSX10.7.sdk -mmacosx-version-min=10.7  -fexceptions';
...
if ispc
  ...
  try
    ...
    if atleast78
      ...
      mex('-f', [matlabroot '/bin/matopts.sh'], 'matfile.c', '-V5')
    else
      mex('-f', [matlabroot '/bin/matopts.sh'], 'matfile.c')
    end  
    % uncomment the line below if you want to build test_flops.exe
    % This program lets you check the flop counts on your processor.
