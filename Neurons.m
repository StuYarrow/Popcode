classdef Neurons
	...
	methods
		...
		function varargout = ssi(obj, n, method, stim, stimOrds, tol, maxit)
			...
			while cont
				...
				if exist('margMask', 'var')
					...
					rCellMarga = cellfun(@(r) r(margMask), rCella, 'UniformOutput', false);
					rCellMarg = repmat(rCellMarga, [stim.n 1]);

					% P(r|s)
					% Subtract out the mean responses
					%subMeanR = cellfun(@minus, rCellMarg, rMeanMargCell, 'UniformOutput', false);
					% Replicate to form a stim.n x stim.n cell array of response vectors
					...
					% P(s|r)
					% Divide joint by marginal P(r)
					pSgR = pRS ./ repmat(pRmarg(iter,:), [stim.n, 1]);
		...
		function varargout = ssi2(obj, n, method, stim, stimOrds, tol, maxit)
			tic
		...
		function varargout = surprise(obj, n, method, stim, stimOrds, tol, maxit)
			...
			if sum(strcmp(method, {'quadrature' 'randMC' 'quasirandMC'})) == 0
				error([method ' is not a valid Isur calculation method'])
			end
			...
			while cont
				if ~mod(iter, 10)
					if continuous
						...
						fprintf('Isur  iter: %d  HF power: %.4e\n', iter, hfPwrIsur)
					else
						fprintf('Isur  iter: %d  smoothness criterion is not used for discontinuous Isur calculations\n', iter)
					end
				end
				...
				if exist('/tmp/haltnow', 'file')
					cont = false;
					disp('Detected /tmp/haltnow, aborting Isur calculation')
				end

				iter = iter + 1;
			end

			fprintf('Isur  iter: %d  elapsed time: %.4f seconds\n', iter - 1, toc)

			switch nargout
			case 1
				varargout = {Isur};
			case 2
				varargout = {Isur (iter - 1)};
			case 3
				varargout = {fullIsur remainderIsur (iter - 1)};
			otherwise
				error('Wrong number of outputs')
			end
		end
